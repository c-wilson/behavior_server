...
def mouse_path(_, filename):
    try:
        mouse, session, date = parse_h5path(filename)
        return "datafiles/{0}/{1}".format(mouse, filename)
    except:
        estr = "Filename does not match template. ({0})".format(filename)
